...
- **Series tracking** for books — a series name + series number, shown under the title in list/grid views (e.g. "Mistborn, #1")
- **Sort title** override, any category — e.g. set it to "Hobbit, The" so a title sorts under H; the catalog's Title sort uses it when present and falls back to the regular title otherwise
...
- **Merge** two items added by the same person into one (unions their formats, fills in blanks from either side, including the Plex link if either side has one)
...
- Item detail page (`/items/[id]`) with a desktop layout (expanded Plex metadata) and a mobile layout, alongside catalog list/grid views and an edit form. Any movie with Digital checked shows its Plex metadata if it's linked to a Plex title, or an explicit "not found on the Plex server" note if it isn't
- Catalog browsing: filter by category + format, added-by, favorites-only, and your own tags; free-text search; sortable; list or grid view with adjustable grid size (music covers render square instead of a poster aspect ratio, and the artist shows above the title instead of below) — filters/view/sort persist via cookies where sensible, and editing/deleting an item returns you to the exact filtered view you came from
...
- **Data layer**: `lib/db/schema.ts` defines `users`, `items` (the catalog entries — `category` is one of `movie | music | book | other`, `formats` is a Postgres text array scoped per category, with a `raw_metadata` jsonb column holding whatever the metadata source returned, plus `isbn`, `series`/`series_number`, `sort_title`, and `plex_rating_key`/`plex_watch_count` for Plex-synced items), `favorites` and `item_tags` (both per-user, unique per item+user pair), `plex_watch_events`, and `settings` (key/value store for admin-configurable API keys and grid pill colors). Migrations live in `drizzle/`, generated from `schema.ts` via `db:generate`. `lib/catalog-values.ts` supplies the distinct creator/series values used for autocomplete.
...
- **Duplicate detection**: `lib/duplicates.ts` groups items by category + normalized title (exact match, not fuzzy — catalog titles come from metadata APIs and are already canonical) for `/duplicates`; the merge logic itself is shared between the manual picker (`mergeItemAction`) and the one-click button there (`quickMergeAction`), both in `app/items/[id]/merge/actions.ts`. That shared `performMerge()` deletes the source row before updating the target so a Plex-linked source's `plexRatingKey` (unique per row) is free to move over without a constraint conflict.
...
`next start` binds to all interfaces on port 3000 by default, not just localhost, so the app is reachable two ways at once with no extra config: plain HTTP directly on port 3000 (e.g. `http://192.168.1.50:3000`) for everyday browsing, and HTTPS through the reverse proxy (e.g. via Caddy or `tailscale serve`) for anything that needs a secure context — namely the camera-based barcode scanner, which browsers refuse to run over plain HTTP. If a firewall (e.g. `ufw`) is enabled, make sure port 3000 is allowed for LAN access.
